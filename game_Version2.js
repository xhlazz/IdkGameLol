...
// Simple web game engine (Vanilla JS, no frameworks)
...
async function loadAssets() {
  ...
  assets.heart = await loadImage('assets/gui/heart.png');
  assets.monster1 = await loadImage('assets/monsters/monster1.png');
  assets.tiles = await loadImage('assets/tiles/tiles.png');
}
...
function loadLevel(n) {
  // For demo, 2 hardcoded levels. For full game, generate or load more
  currentLevel = n;
  const levels = [
    {
      tiles: [
...
function drawPlayer() {
  ...
  if (player.character === "melkin") {
    ...
    // If attacking, draw a "punch" sprite
